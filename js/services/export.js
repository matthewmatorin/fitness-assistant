...
async function exportData(tableName, format = 'csv') {
    try {
        ...
        if (format === 'csv') {
            ...
            // For Excel, just use CSV format but keep .csv extension for compatibility
            const csv = convertToCSV(data);
            downloadFile(csv, `${tableName}.csv`, 'text/csv');
            showToast(`${tableName} exported as CSV (Excel compatible)`, 'info');
            return; // Early return to avoid duplicate success message
        }
...
async function exportLocalData(dataType, format = 'csv') {
    try {
        ...
        if (format === 'csv') {
            ...
            // Export as CSV with .csv extension (Excel can open CSV files perfectly)
            const csv = convertToCSV(data);
            downloadFile(csv, `${filename}.csv`, 'text/csv');
            showToast(`${dataType} exported as CSV (Excel compatible) from local data!`, 'info');
            return; // Early return to avoid duplicate success message
        }
...
// Convert data array to standard CSV format
...
// Convert data to Excel-compatible CSV format with BOM
function convertToExcelCSV(data) {
    ...
    // Add data rows with better Excel compatibility
    for (const row of data) {
        const values = headers.map(header => {
            const value = row[header];
