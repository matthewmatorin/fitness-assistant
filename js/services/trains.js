...
async function getTrainsToNYC() {
    try {
        ...
        if (shouldUseAPI()) {
            console.log('Attempting Transit.land API for toNYC...');
...
async function getTrainsFromNYC() {
    try {
        // Try Transit.land API first if we have an API key and haven't hit limits
        if (shouldUseAPI()) {
            console.log('Attempting Transit.land API for fromNYC...');
...
async function tryTransitlandAPI(direction) {
    try {
        ...
        // Use schedule_stop_pairs endpoint instead of trips
        const apiUrl = `${TRANSIT_LAND_API_BASE}/schedule_stop_pairs?` + new URLSearchParams({
            origin_onestop_id: originId,
            destination_onestop_id: destinationId,
            date: today,
            service_from_date: today,
            service_before_date: today,
        ...
        if (data.schedule_stop_pairs && data.schedule_stop_pairs.length > 0) {
            ...
            return parseTransitLandSchedulePairs(data.schedule_stop_pairs, direction);
        } else {
