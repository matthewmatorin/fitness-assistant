...
function loadWeightDetails() {
    ...
    document.getElementById('weight-this-week').textContent = thisWeekAvg ? `${thisWeekAvg.toFixed(1)} lbs` : '--';
    document.getElementById('weight-last-week').textContent = lastWeekAvg ? `${lastWeekAvg.toFixed(1)} lbs` : '--';
    document.getElementById('weight-monthly-avg').textContent = monthlyAvg ? `${monthlyAvg.toFixed(1)} lbs` : '--';
...
function calculatePredictions() {
    ...
    if (weightData.length >= 3) {
        const recentWeights = weightData.slice(0, 8); // Last 8 entries
    ...
    if (workoutData.length >= 4) {
        const last4Weeks = getLast4WeeksWorkouts();
        const avgWorkoutsPerWeek = last4Weeks.reduce((sum, week) => sum + week.length, 0) / 4;
        const thisWeek = getWorkoutsThisWeek();
        
        predictions.workoutTrend = {
            currentWeeklyAvg: avgWorkoutsPerWeek.toFixed(1),
            thisWeekCount: thisWeek,
            consistencyScore: (thisWeek >= avgWorkoutsPerWeek ? 'improving' : 'declining'),
            projectedMonthlyWorkouts: Math.round(avgWorkoutsPerWeek * 4.3)
        };
    }
    
    // Walking improvement predictions
    const walkingStats = getWalkingStatsComparison();
    if (walkingStats.thisWeek > 0 || walkingStats.lastWeek > 0) {
        predictions.walkingTrend = {
            weeklyChange: walkingStats.change,
            currentWeeklyMinutes: walkingStats.thisWeek,
            projectedMonthlyMinutes: Math.round(walkingStats.thisWeek * 4.3),
            improvementRate: walkingStats.lastWeek > 0 ? 
                ((walkingStats.change / walkingStats.lastWeek) * 100).toFixed(1) + '%' : 'new habit'
        };
    }
    
    // Running improvement predictions
    const runningStats = getRunningStatsComparison();
    if (runningStats.thisWeek > 0 || runningStats.lastWeek > 0) {
        predictions.runningTrend = {
            weeklyChange: runningStats.change.toFixed(1),
            currentWeeklyMiles: runningStats.thisWeek.toFixed(1),
            projectedMonthlyMiles: (runningStats.thisWeek * 4.3).toFixed(1),
            improvementRate: runningStats.lastWeek > 0 ? 
                ((runningStats.change / runningStats.lastWeek) * 100).toFixed(1) + '%' : 'new habit'
        };
    }
...
function getLast4WeeksWorkouts() {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        const weekWorkouts = workoutData.filter(w => {
            ...
            return workoutDate >= monday && workoutDate <= weekEnd;
        });
...
function setupEventListeners() {
    ...
    setTimeout(() => {
        ...
        // Initialize predictive insights for dashboard
...
function getWorkoutsThisWeek() {
    ...
    const thisWeekWorkouts = workoutData.filter(workout => {
...
function getWorkoutsLastWeek() {
    ...
    return lastWeekWorkouts.length;
}
