...
// Enhanced Weight Chart with Moving Average Comparison
function createInsightsChart() {
    ...
    // Calculate moving average and current vs average comparison
    const { movingAverage, currentVsAverage } = calculateMovingAverageComparison(filteredWeights, selectedDays);
    
    // Update period change display with average comparison
    updatePeriodChangeWithAverage(filteredWeights, selectedDays, currentVsAverage);
    
    // Create the enhanced chart
    createEnhancedWeightChart(ctx, filteredWeights, movingAverage, currentVsAverage);
}

// Calculate moving average and current comparison
function calculateMovingAverageComparison(filteredWeights, selectedDays) {
    if (filteredWeights.length === 0) return { movingAverage: 0, currentVsAverage: null };
    
    const weights = filteredWeights.map(w => w.weight);
    const currentWeight = filteredWeights[filteredWeights.length - 1].weight; // Most recent
    ...
    switch (selectedDays) {
        case '7':
            windowSize = Math.min(3, weights.length); // 3-day window for 7-day period
            break;
        case '14':
            windowSize = Math.min(5, weights.length); // 5-day window for 14-day period
            break;
        case '30':
            windowSize = Math.min(7, weights.length); // 7-day window for 30-day period
            break;
        case '60':
            windowSize = Math.min(10, weights.length); // 10-day window for 60-day period
            break;
        case '90':
            windowSize = Math.min(14, weights.length); // 14-day window for 90-day period
            break;
        case 'all':
            windowSize = Math.min(21, weights.length); // 21-day window for all-time
            break;
        default:
            windowSize = Math.min(7, weights.length);
    }
    
    // Calculate simple moving average
    if (weights.length >= windowSize) {
        const recentWeights = weights.slice(-windowSize);
        movingAverage = recentWeights.reduce((sum, weight) => sum + weight, 0) / recentWeights.length;
    } else {
        // Not enough data for window, use simple average
        movingAverage = weights.reduce((sum, weight) => sum + weight, 0) / weights.length;
    }
    
    // Calculate comparison
    const difference = currentWeight - movingAverage;
    const currentVsAverage = {
        current: currentWeight,
        average: movingAverage,
        difference: difference,
        isBelow: difference < 0,
        percentDiff: Math.abs(difference / movingAverage) * 100
    };
    
    return { movingAverage, currentVsAverage };
}

// Update period change display with average comparison
function updatePeriodChangeWithAverage(filteredWeights, selectedDays, currentVsAverage) {
    const periodChangeElement = document.getElementById('weight-period-change');
    
    if (filteredWeights.length >= 2 && periodChangeElement && currentVsAverage) {
        const startWeight = filteredWeights[0].weight;
        const endWeight = filteredWeights[filteredWeights.length - 1].weight;
        const periodChange = endWeight - startWeight;
        const periodName = selectedDays === 'all' ? 'overall' : `${selectedDays}-day`;
        
        // Format the comparison text
        const comparisonText = currentVsAverage.isBelow 
            ? `${Math.abs(currentVsAverage.difference).toFixed(1)} lbs below`
            : `${currentVsAverage.difference.toFixed(1)} lbs above`;
        
        const comparisonColor = currentVsAverage.isBelow ? '#10b981' : '#ef4444';
        
        let periodText = '';
        if (periodChange < 0) {
            periodText = `<span style="color: #10b981;">↓ ${Math.abs(periodChange).toFixed(1)} lbs lost (${periodName})</span>`;
        } else if (periodChange > 0) {
            periodText = `<span style="color: #ef4444;">↑ ${periodChange.toFixed(1)} lbs gained (${periodName})</span>`;
        } else {
            periodText = `<span style="color: #667eea;">No change (${periodName})</span>`;
        }
        
        periodChangeElement.innerHTML = `
            ${periodText}<br>
            <span style="color: ${comparisonColor}; font-size: 14px;">
                Current: ${comparisonText} ${getMovingAverageLabel(filteredWeights.length)} average
            </span>
        `;
...
// Create enhanced chart with moving average line
function createEnhancedWeightChart(ctx, filteredWeights, movingAverage, currentVsAverage) {
    const labels = filteredWeights.map(w => w.date.toLocaleDateString());
    const weightData = filteredWeights.map(w => w.weight);
    
    // Create moving average line data (horizontal line)
    const averageLineData = new Array(filteredWeights.length).fill(movingAverage);
    
    // Determine colors based on current vs average
    const currentPointColor = currentVsAverage && currentVsAverage.isBelow ? '#10b981' : '#ef4444';
            ...
            datasets: [
                {
                    ...
                    tension: 0.1,
                    fill: false,
                    pointBackgroundColor: weightData.map((_, index) => {
                        // Highlight the last point (current weight) with special color
                        return index === weightData.length - 1 ? currentPointColor : '#000000';
                    }),
                    pointBorderColor: '#ffffff',
                    pointBorderWidth: 2,
                    pointRadius: weightData.map((_, index) => {
                        // Make the last point (current weight) larger
                        return index === weightData.length - 1 ? 8 : 5;
                    })
                },
                {
                    label: `${getMovingAverageLabel(filteredWeights.length)} Average`,
                ...
                tooltip: {
                    callbacks: {
                        afterBody: function(tooltipItems) {
                            ...
                            if (dataIndex === weightData.length - 1 && currentVsAverage) {
                                const diff = currentVsAverage.isBelow ? 
                                    `${Math.abs(currentVsAverage.difference).toFixed(1)} lbs below average` :
                                    `${currentVsAverage.difference.toFixed(1)} lbs above average`;
                                return [`Current vs Average: ${diff}`];
                            }
                            return [];
                        }
                    }
...
// Get appropriate moving average label based on data length and period
function getMovingAverageLabel(dataLength) {
    if (dataLength <= 7) return '3-day';
    if (dataLength <= 14) return '5-day';
    if (dataLength <= 30) return '7-day';
    if (dataLength <= 60) return '10-day';
    if (dataLength <= 90) return '14-day';
    return '21-day';
}
